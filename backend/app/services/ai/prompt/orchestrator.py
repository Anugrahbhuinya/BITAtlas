...
class PromptOrchestrator:
    ...
    def assemble_prompt(self, context: PromptContext, metadata: PromptMetadata) -> PromptSchema:
        ...
        hallucination_rules = ""
        # Educational intent uses Gemini's general knowledge — do NOT inject BIT-restricted guard
        is_educational_intent = template_name == "educational"
        if metadata.hallucination_guard_enabled and not is_educational_intent:
...
Nearby Facilities: {facilities_str}"""
        ...
        if hallucination_rules:
            builder.set_safety(hallucination_rules)
